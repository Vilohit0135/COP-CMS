...
export default function HomeProgramExpertsSectionPage() {
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) alert("Program experts section saved successfully");
      else alert(data.error || "Failed to save");
    } catch (error) {
      alert("Error saving program experts section");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold text-slate-800">Program Experts Manager</h1>
            <p className="text-sm text-slate-400 mt-1">Manage homepage program expert profiles</p>
          </div>
